...
const KEY_LAWS: { code: string; sub: string; detail: string; gradient: string; url: string }[] = [
  { code: "WPA 1972 (Amended 2022)", sub: "4 schedules · 2,600+ species", detail: "India’s primary wildlife law. Schedule I gives absolute protection to 900+ species — tigers, elephants, rhinos, lions. Section 51: 3–7 years imprisonment + minimum ₹25,000 fine. Bail is barred under Section 51A for serious offences. Amended in 2022 to consolidate 6 schedules into 4. Report violations to Forest Helpline 1926 or WCCB +91-11-26182484.", gradient: "from-emerald-600/80 to-teal-700/80", url: "https://indiacode.nic.in/bitstream/123456789/1726/1/197253.pdf" },
  { code: "PCA Act 1960", sub: "Animal welfare · AWBI · Sec 11", detail: "Prevention of Cruelty to Animals Act 1960 — the core animal welfare law. Section 11 lists all cruelty offences: beating, overloading, starving, keeping in cramped conditions, using in entertainment. Enforced by AWBI (Animal Welfare Board of India) and local police. First offence fine ₹50–₹250; repeat offences can lead to imprisonment up to 3 months. Contact AWBI: +91-129-2555700 or PETA India: +91-22-40727382.", gradient: "from-amber-600/80 to-orange-700/80", url: "https://awbi.gov.in/awbi-pdf/Prevention_of_cruelty_to_animals_act%201960.pdf" },
  { code: "BNS Section 325 (2024)", sub: "Replaces IPC 428/429", detail: "Bharatiya Nyaya Sanhita Section 325 replaced IPC Sections 428 and 429 in 2024. Covers mischief by killing, poisoning, maiming or rendering useless any animal. Punishment: up to 5 years imprisonment + fine. Applies to stray dog poisoning, livestock harm, and deliberate killing of any animal. File an FIR at your nearest police station under BNS Sec 325.", gradient: "from-rose-600/80 to-red-700/80", url: "https://legislative.gov.in/sites/default/files/2023-12/BNS.pdf" },
  { code: "Forest Conservation Act 1980", sub: "Diversion · clearance · NPV", detail: "Restricts diversion of forest land for non-forest purposes (mining, construction, roads). Any diversion requires prior Central Government approval. Violators face up to 15 days imprisonment. Net Present Value (NPV) must be paid as compensation for diverted forest land. Report illegal forest encroachment to your State Forest Department or Forest Helpline 1926.", gradient: "from-green-700/80 to-emerald-800/80", url: "https://indiacode.nic.in/bitstream/123456789/1474/1/198069.pdf" },
  { code: "Biological Diversity Act 2002", sub: "Bio-resources · NBA · access", detail: "Regulates access to India’s biological resources and associated traditional knowledge. Any foreign entity or NRI accessing bio-resources must obtain prior approval from the National Biodiversity Authority (NBA). Indian companies must notify State Biodiversity Boards. Violations: imprisonment up to 5 years or fine up to ₹10 lakh or both. Covers plants, animals, microorganisms and their derivatives.", gradient: "from-cyan-600/80 to-blue-700/80", url: "https://nbaindia.org/uploaded/docs/Biodiversity%20act,%202002.pdf" },
  { code: "EPA 1986", sub: "Environment umbrella law", detail: "Environment Protection Act 1986 is the overarching environmental law empowering the Central Government to protect and improve the environment. Sets standards for emissions and discharges. Authorities can inspect, take samples, and shut down polluting units. Penalty: up to 5 years imprisonment + ₹1 lakh fine, extendable to 7 years for continued violation. Report environmental damage to your State Pollution Control Board.", gradient: "from-indigo-600/80 to-violet-700/80", url: "https://cpcb.nic.in/uploads/Publications/Publication_66_environment_protection_act_1986.pdf" },
];
...
export default function WildGuardChat() {
  ...
  // Re-localize slogan + scenarios when language changes, save current chat, then start new chat
  ...
  useEffect(() => {
    ...
    setRotateKey((k) => k + 1);
    // Skip on initial mount (prevLangRef not set yet)
    if (prevLangRef.current !== null && prevLangRef.current !== language) {
      // Active chat is already in conversations state — just deactivate
      setActiveId(null);
      setInput("");
    }
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-[#f4f1e8]">
          ...
          <header className="px-4 sm:px-8 pt-4 sm:pt-5 pb-3 flex items-start gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end">
              {/* Past Chats button — dimmed with overlay when signed out */}
              <div className="relative" onClick={(e) => e.stopPropagation()}>
                <div className={`relative ${!user ? "opacity-50" : ""}`}>
                  <PillBtn icon={History} title="Past Chats" sub="your saved conversations"
                    onClick={() => {
                      if (!user) { toast.info("Sign in to access past chats"); return; }
                      setPastOpen((v) => !v); setLangOpen(false); setUserMenu(false);
                    }} active={pastOpen} />
                  {!user && <div className="absolute inset-0 rounded-md cursor-not-allowed" />}
                </div>
                {pastOpen && user && (
                  <Popover>
              ...
              {/* Share Chat — enabled, shows toast if no messages */}
              <PillBtn
                icon={Share2}
                title="Share Chat"
            ...
            >
              ...
              >
                ...
                <a href={law.url} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 mt-5 text-[12px] font-semibold text-white/80 hover:text-white underline underline-offset-2 transition">
                  <ArrowRight className="h-3.5 w-3.5" /> Read full Act on India Code
                </a>
              </div>
            </div>
          );
